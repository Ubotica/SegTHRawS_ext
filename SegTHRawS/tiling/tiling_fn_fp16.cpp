...
// // This code uses the heap memory to fit the entire multiband image
...
const int numPatchesHeight = full_image_height / patch_height;
const int numPatchesWidth = full_image_width / patch_width;
...
int main(int argc, char* argv[]) {
    ...
    // float16 final_image[full_image_height][full_image_width][Channels];

    std::vector<std::vector<std::vector<float16>>>* final_image = new std::vector<std::vector<std::vector<float16>>>(full_image_height, std::vector<std::vector<float16>>(full_image_width, std::vector<float16>(Channels)));
    ...
    if (image_shape_type == "interlinear"){ // Interlinear format (height, width, channels)
    ...
    else if (image_shape_type == "planar"){ // Planar format (channels, height, width)
        ...
        for (int idx = 0; idx < float_buffer.size(); ++idx) {
            ...
            if (width_idx == full_image_width) {
                width_idx = 0;
                ++height_idx;
                if (height_idx == full_image_width) {
                    height_idx = 0;
                    ++channel_idx;
                }
            }
        ...
        std::cout << "Error: Image shape type must be interlinear or planar " << std::endl;
    }
