...
def save_as_binary(output_path: str,
                   image: np.ndarray
                   ):
...
def convert_interlinear2planar(input_image_path: str,
                               output_folder_path: str,
                               input_dtype,
                               output_dtype,
                               input_shape: list = [256,256,3],
                               ):
    if not output_dtype:
    ...
    image = read_binary_image(image_path=input_image_path,dtype = input_dtype,shape=input_shape)

    image = np.transpose(image,(2,0,1)).astype(output_dtype)

    save_as_binary(output_path=output_image_path,image=image)


    # output_folder_path = os.path.join(os.getcwd(),'new_images')
    # os.makedirs(output_folder_path,exist_ok=True)

    # images_path = [os.path.join(os.getcwd(),'images',image_name) for image_name in os.listdir(os.path.join(os.getcwd(),'images'))]

    # for image_path in images_path:
    #     convert_interlinear2planar(input_image_path=image_path,
