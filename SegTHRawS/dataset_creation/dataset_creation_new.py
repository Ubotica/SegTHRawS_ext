...
sys.path.insert(1, os.path.dirname(os.path.dirname(__file__))) #This line imports the  SegTHRawS directory to get access to utils
    ...
    for scene_path in scenes_paths:
        ...
        if scene_name[-3:]!='_NE':
            ...
            for granule_idx,_ in enumerate(granules_paths):
                ...
                else: #Using downsampling False changes the DN. Even including negative values
                    raw_coreg_granule = raw_coreg_granule.as_tensor().numpy() #Produces negative values in the images
                ...
                for i in range(multispectral_patches.shape[0]):
                    for j in range(multispectral_patches.shape[1]):
                        ...
                        # print(scene_name,NIR_SWIR_patch.max(),NIR_SWIR_patch.min())
                        ...
                        if not (NIR_SWIR_patch.max() == 0 or RGB_patch.max() == 0 or VNIR_patch.max() == 0 or NIR_1_patch.max() == 0):
                            ...
                            if np.any(voting_2): #Intersection and voting 3 not included, because voting_2 condition is enough to consider a potential event.
