...
class SegTHRawSTrainModel(pl.LightningModule):
    ...
    def shared_step(self, batch, stage: str) -> torch.Tensor:
        ...
        self.stage_outputs[stage]["fbeta_old"].append(fbeta_score)
        self.stage_outputs[stage]["IoU_old"].append(IoU_score)
        ...
        if len(IoU_mask_event)>0:        
            ...
            if torch.any(IoU_mask_event>0.5):
                ...
                if torch.any(IoU_mask_event>0.75):
                    ...
                    self.stage_outputs[stage]["IoU_event_0.75"].append(IoU_0_75_events)
        ...
        if len(IoU_mask_notevent)>0:        
            ...
            if torch.any(IoU_mask_notevent>0.5):
                ...
                if torch.any(IoU_mask_notevent>0.75):
                    ...
                    self.stage_outputs[stage]["IoU_notevent_0.75"].append(IoU_0_75_notevents)
    ...
    def shared_epoch_end(self, stage: Any):
        ...
        fbeta_old = torch.mean(torch.tensor(
            [fbeta_score for fbeta_score in self.stage_outputs[stage]["fbeta_old"]
             ]))
        ...
        IoU_old = torch.mean(torch.tensor(
                [IoU_score for IoU_score in self.stage_outputs[stage]["IoU_old"]
                 ]))
        ...
        IoU_event_0_75 = torch.mean(torch.tensor(
                [IoU_score for IoU_score in self.stage_outputs[stage]["IoU_event_0.75"]
                 ]))
        ...
        IoU_notevent_0_75 = torch.mean(torch.tensor(
                [IoU_score for IoU_score in self.stage_outputs[stage]["IoU_notevent_0.75"]
                 ]))
