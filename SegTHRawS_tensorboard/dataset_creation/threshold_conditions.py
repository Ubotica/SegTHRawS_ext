...
'''
empty_pixels_pattern corresponds to the mask that contains the common part for all the bands.
Example:
    empty_pixels_pattern = np.all(raw_coreg_granule != 0,axis=2)
where raw_coreg_granule is the the output from the coarse coregistration function, expressed as a tensor and converted into an numpy array. The expected shape is (height,width,channel)
'''
