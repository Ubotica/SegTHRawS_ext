...
def split_events(
    ...
    if train_split_ratio + val_split_ratio + test_split_ratio != 1:
        ...
        if train_split_ratio and not val_split_ratio and not test_split_ratio:
            print(
                "Validation and test split ratios were not defined. Assuming equal split"
            )
    ...
    if not new_dataset_path:
        new_dataset_path = os.path.join(
            os.path.dirname(dataset_path), "train_random_split_dataset"
        )
        create_dataset_folders(new_dataset_path)

    train_max_idx = np.round(len(os.listdir(masks_path)) * train_split_ratio).astype(
        int
    )

    val_max_idx = train_max_idx + np.round(
        len(os.listdir(masks_path)) * val_split_ratio
    ).astype(int)
        ...
        if idx < train_max_idx:
            ...
            with open(
                os.path.join(
                    ...
                    "train",
                    "masks",
                    mask_name.replace("_mask_weakly.pkl", "_mask.bin"),
                ),
                "wb",
            ) as file:
                mask.tofile(file)
    ...
    print(
        f"    Events generated: {train_idx} TRAINING, {val_idx} VAL, and {test_idx} TEST "
    )
...
def split_notevents(
    ...
    test_max_cont_end_loop = len(
        os.listdir(masks_path)
    )  # Last index of where the data will be gathered
    train_max_cont_end_loop = np.round(
        test_max_cont_end_loop * train_split_ratio
    ).astype(int)

    val_max_cont_end_loop = train_max_cont_end_loop + np.round(
        test_max_cont_end_loop * val_split_ratio
    ).astype(int)
    ...
    n_scenes = len(
        np.unique([re.match(r"(.+)_G", granule).group(1) for granule in granules])
    )
    ...
    for image_name in notevents_paths_shuffle:
        ...
        if (
            scene_names_list.count(scene_name) < n_images_per_granule
        ):  # This tries to get images from multiple scenes, to avoid focusing only on one
            ...
            elif (cont_end_loop > train_max_cont_end_loop) and (
                cont_end_loop <= val_max_cont_end_loop
            ):
            ...
            elif (cont_end_loop > val_max_cont_end_loop) and (
                cont_end_loop <= test_max_cont_end_loop
            ):
    ...
    missing_test_images = np.abs(
        test_max_cont_end_loop - val_max_cont_end_loop - cont_test
    )
    ...
    print(
        f"Not events generated: {cont_train} TRAINING, {cont_val} VAL, and {cont_test} TEST "
    )


def generate_random_split_dataset(
