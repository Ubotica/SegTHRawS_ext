"""
 ...
 - The columns of the csv output file represent: Patch name, number of event pixels, 
      number of potential event pixels, number of not event pixels, percentage of events pixels,
      percentage of potential events pixels, and percentage of not events pixels.
...
from .constants import SEGTHRAWS_DIRECTORY

from ..utils import read_binary_image


def compute_seg_mask_statistics(dataset_path: str = None):

    if not dataset_path:
        dataset_path = os.path.join(SEGTHRAWS_DIRECTORY, "dataset")
    ...
    csv_headers = [
        "Name ",
        "Event pixels ",
        "Potential event pixels ",
        "Not event pixels ",
        "Proportion events ",
        "Proportion potential events ",
        "Proportion not events ",
    ]
    ...
    for mask_name in os.listdir(segmentation_masks):
        ...
        mask_segmentation = read_binary_image(
            segmentation_mask_path, dtype=np.float32, shape=[PATCH_SIZE, PATCH_SIZE, 1]
        )
        ...
        proportion_event_pixels = np.round(
            n_event_pixels
            / (n_event_pixels + n_potential_event_pixels + n_not_event_pixels)
            * 100,
            3,
        )
        proportion_potential_event_pixels = np.round(
            n_potential_event_pixels
            / (n_event_pixels + n_potential_event_pixels + n_not_event_pixels)
            * 100,
            3,
        )
        proportion_not_event_pixels = np.round(
            n_not_event_pixels
            / (n_event_pixels + n_potential_event_pixels + n_not_event_pixels)
            * 100,
            3,
        ...
        statistic_list.append(
            [
                ...
                n_potential_event_pixels,
                n_not_event_pixels,
                proportion_event_pixels,
                proportion_potential_event_pixels,
                proportion_not_event_pixels,
            ]
        )
        ...
        with open(csv_path, "a") as f_object:
            ...
            writer_object.writerow(
                [
                    re.match(r"(.+)_mask", mask_name).group(1),
                    n_event_pixels,
                    n_potential_event_pixels,
                    n_not_event_pixels,
                    proportion_event_pixels,
    ...
    with open(csv_path, "a") as f_object:
        ...
        writer_object.writerow(
            [
                "Mean values ",
                np.round(np.mean(statistic_list, axis=0), 3)[0],
        ...
        writer_object.writerow(
            [
                "Std values ",
                np.round(np.std(statistic_list, axis=0), 3)[0],
        ...
        writer_object.writerow(
            [
                "Total sum ",
                int(np.sum(statistic_list, axis=0)[0]),
