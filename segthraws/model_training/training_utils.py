...
class SegTHRawSDataset(Dataset):
    ...
    def __create_dataset(self) -> dict:
        ...
        for image_name in images_path_shuffle:
            ...
            dict_paths["mask"].append(
                os.path.join(
                    os.path.dirname(images_path),
                    "masks",
                    image_name.replace(".bin", "_mask.bin"),
                )
            )
    ...
    def __getitem__(self, idx) -> tuple:
        ...
        image = read_binary_image(
            image_path=self._images[idx], dtype=np.float32, shape=[256, 256, 3]
        )

        mask = read_binary_image(
            image_path=self._masks[idx], dtype=np.float32, shape=[256, 256, 1]
        )
...
def get_preprocessing(preprocessing_fn):
    ...
    preprocessing_fn : Any
        pre_processing normalization function, which substracts the mean
          and std of the input images and normalize.

    Outputs
    -------
    transform: albumentations.Compose
        Pre-processing transformation needed to normalize the input data
          to that of the pre-trained weights.
...
class SegTHRawSDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.eval_augmentation = (
            augmentation  # At the moment, using the same as for training
        )
        self.preprocessing = (
            preprocessing  # Needed for pre-trained backbones in imagenet
        )
...
class SegTHRawSTrainModel(pl.LightningModule):
    ...
    mean_imagenet_imgs : list
        Mean of the imagenet images required for saving testing output masks when
          pre-trained weights are used
    std_imagenet_imgs : list
        Standard deviation of the imagenet images required for saving testing output masks
          when pre-trained weights are used
    lr_scheduler : bool
    ...
    def shared_step(self, batch, stage: str,batch_idx: Any = 0) -> torch.Tensor:
        x, y = batch
        ...
        fbeta_score = self.metrics["fbeta_score"](
            predictions, y
        )  # This includes ignore index inside the function
        ...
        for idx, mask_batch in enumerate(y):
            if torch.any(mask_batch != 0):
                if self.weakly:
                    ...
                    iou_mask_event.append(
                        self.metrics["iou"](predictions_event, mask_batch_event)
                    )
                    fbeta_events_list.append(
                        self.metrics["fbeta_score"](predictions_event, mask_batch_event)
                    )
                else:
                    iou_mask_event.append(
                        self.metrics["iou"](predictions[idx], mask_batch)
                    )
                    fbeta_events_list.append(
                        self.metrics["fbeta_score"](predictions[idx], mask_batch)
                    )
            else:
                if self.weakly:
                    not_weakly_classes = mask_batch != -1
                    ...
                    iou_mask_notevent.append(
                        self.metrics["iou"](predictions_notevent, mask_batch_notevent)
                    )
                else:
                    iou_mask_notevent.append(
                        self.metrics["iou"](predictions[idx], mask_batch)
        ...
        if len(iou_mask_event) > 0:
            ...
            if torch.any(iou_mask_event > 0.5):
                iou_0_5_events = torch.mean(
                    torch.tensor([item for item in iou_mask_event if item > 0.5])
                )
                self.stage_outputs[stage]["iou_event_0.5"].append(iou_0_5_events)

                if torch.any(iou_mask_event > 0.9):
                    iou_0_9_events = torch.mean(
                        torch.tensor([item for item in iou_mask_event if item > 0.9])
                    )
        ...
        if len(iou_mask_notevent) > 0:
            ...
            if torch.any(iou_mask_notevent > 0.5):
                iou_0_5_notevents = torch.mean(
                    torch.tensor([item for item in iou_mask_notevent if item > 0.5])
                )
                self.stage_outputs[stage]["iou_notevent_0.5"].append(iou_0_5_notevents)

                if torch.any(iou_mask_notevent > 0.9):
                    iou_0_9_notevents = torch.mean(
                        torch.tensor([item for item in iou_mask_notevent if item > 0.9])
                    )
                    self.stage_outputs[stage]["iou_notevent_0.9"].append(
                        iou_0_9_notevents
                    )
        ...
        if stage == "test" and self.save_imgs:
            test_masks_path = os.path.join(
                self.model_folder_path, "test_masks_comparison"
            )
            ...
            for batch_idx in range(x.shape[0]):
                ...
                prediction_mask = np.transpose(
                    predictions[batch_idx].cpu().detach().numpy(), (1, 2, 0)
                )
                ...
                plt.suptitle(
                    f" Test masks comparison {test_idx+batch_idx}", fontsize=14
                )
                ax[0].imshow(image)
                ax[1].imshow(mask)
                ...
                plt.savefig(
                    os.path.join(
                        test_masks_path, f"test_mask_comparison_{test_idx+batch_idx}"
                    )
                )
                plt.close()
    ...
    def shared_epoch_end(self, stage: Any):
        loss = torch.mean(
            torch.tensor([loss for loss in self.stage_outputs[stage]["loss"]])
        )
        ...
        fbeta_event = torch.mean(
            torch.tensor(
                [
                    fbeta_score
                    for fbeta_score in self.stage_outputs[stage]["fbeta_event"]
                ]
            )
        )

        iou = torch.mean(
            torch.tensor([iou_score for iou_score in self.stage_outputs[stage]["iou"]])
        )

        iou_event = torch.mean(
            torch.tensor(
                [iou_score for iou_score in self.stage_outputs[stage]["iou_event"]]
            )
        )

        iou_event_0_5 = torch.mean(
            torch.tensor(
                [iou_score for iou_score in self.stage_outputs[stage]["iou_event_0.5"]]
            )
        )

        iou_event_0_9 = torch.mean(
            torch.tensor(
                [iou_score for iou_score in self.stage_outputs[stage]["iou_event_0.9"]]
            )
        )

        iou_notevent = torch.mean(
            torch.tensor(
                [iou_score for iou_score in self.stage_outputs[stage]["iou_notevent"]]
            )
        )

        iou_notevent_0_5 = torch.mean(
            torch.tensor(
                [
                    iou_score
                    for iou_score in self.stage_outputs[stage]["iou_notevent_0.5"]
                ]
            )
        )

        iou_notevent_0_9 = torch.mean(
            torch.tensor(
                [
                    iou_score
                    for iou_score in self.stage_outputs[stage]["iou_notevent_0.9"]
                ]
            )
        )
    ...
    def training_step(self, batch: Any, batch_idx: Any = 0):
        return self.shared_step(batch=batch, stage="train",batch_idx=batch_idx)

    def validation_step(self, batch: Any, batch_idx: Any = 0):
        return self.shared_step(batch=batch, stage="val",batch_idx=batch_idx)

    def test_step(self, batch: Any, batch_idx: Any):
        return self.shared_step(batch=batch, stage="test",batch_idx=batch_idx)
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0):
        ...
        image = image * np.array(self.std_imagenet_imgs) + np.array(
            self.mean_imagenet_imgs
        )
...
class SegTHRawSModel(pl.LightningModule):
    ...
    def __init__(
        ...
        if activation == "sigmoid":
            ...
            raise ValueError(f"Activation should be sigmoid" f"; got {activation}")
    ...
    def test_step(self, batch: Any, batch_idx: Any):
        ...
        self.log(
            f"{stage}_fbeta", fbeta_score, prog_bar=True, on_step=False, on_epoch=True
        )
