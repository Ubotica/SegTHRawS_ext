...
def train_fn(
    ...
    callbacks: list
        Callbacks used for training the model.
        These include ModelCheckpoint, EarlyStopping, and LearningRateMonitoring
...
def test_fn(
    ...
    trained_model = SegTHRawSTrainModel.load_from_checkpoint(
        checkpoint_path=checkpoint_path, model=model, loss_fn=loss, save_imgs=save_imgs
    )
    trained_model.eval()
...
def predict_fn(
    ...
    trained_model = SegTHRawSTrainModel.load_from_checkpoint(
        checkpoint_path=checkpoint_path, model=model, loss_fn=loss
    )
    trained_model.eval()
