...
@copyright 2024 see license file for details
 ...
 - This sript includes the main execution file for training, testing and retraining segmentation models. 
 - It was designed to adapt multiple input combinations  
 - The main outputs are the trained models in both PyTorch and ONNX format, as well as the training and testing TensorBoard logs
"""
...
def model_train_test(
    ...
    data_path = os.path.join(datasets_path,f'train_{data_split}_split{weakly_str}_{band_list_str}_dataset')
    if os.path.isdir(data_path):
        model_name_path = os.path.join(MAIN_DIRECTORY,'models',f'{model_name}{encoder_str}{encoder_weights_str}{n_filters_str}_fp{precision[:2]}_{loss_name}_gamma{gamma}_alpha{alpha}{lr_str}{lr_scheduler_str}_{data_split}{weakly_str}_{band_list_str}')
    else:
        raise FileNotFoundError(f"The training dataset folder couldn't be found at: {data_path}")
    ...
    logger = TensorBoardLogger(f"{model_main_path}/logs", name=f"{os.path.basename(model_seed_path)}",default_hp_metric=False)
...
def model_testing(
    ...
    data_path = os.path.join(datasets_path,f'train_{data_split}_split{weakly_str}_{band_list_str}_dataset')
    if os.path.isdir(data_path):
        model_name_path = os.path.join(models_path,f'{model_name}{encoder_str}{encoder_weights_str}{n_filters_str}_fp{precision[:2]}_{loss_name}_gamma{gamma}_alpha{alpha}{lr_str}{lr_scheduler_str}_{data_split}{weakly_str}_{band_list_str}')
    else:
        raise FileNotFoundError(f"The training dataset folder couldn't be found at: {data_path}")
    ...
    model_checkpoint_path = [os.path.join(model_main_path,file) for file in os.listdir(model_main_path) if file.endswith('.ckpt')][0]
...
def model_re_train_test(
    ...
    last_epoch_saved = int(re.search(r"=(\d+)\.ckpt",model_checkpoint_path).group(1))

    trained_model = SegTHRawSTrainModel.load_from_checkpoint(checkpoint_path=model_checkpoint_path,model=model,loss_fn=loss,last_saved_epoch=last_epoch_saved)
