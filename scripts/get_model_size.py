...
def get_model_size(model: nn.Module, output_format: str = "MB") -> None:
    ...
    output_format: str
        String that idicates the desired whether the model size is
          expressed in MB or in number of parameteres
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Function that obtains the expected size of the model"
    )
    ...
    parser.add_argument(
        "--n_filters",
        type=int,
        help="Number of filter used for the modified U-Net network",
        default=32,
    )
    parser.add_argument(
        "--output_format",
        type=str,
        help="Determine the output format: MB or number of parameters. "
        'Options: "MB" OR "params"',
        default="params",
        choices=["MB", "params"],
    )
