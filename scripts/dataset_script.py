...
from segthraws.dataset_creation.random_split_seg_dataset import (
    generate_random_split_dataset,
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        help="Path to the raw THRawS data directory",
        default=thraws_data_path,
    )
    parser.add_argument(
        "--geo_split",
        type=str,
        help="Decide if geographical split is applied. 1: YES; 0: NO",
        default=1,
        choices=[0, 1],
    )
    parser.add_argument(
        "--new_dataset_path",
        type=str,
        help="Path where the new dataset wants to be generated.",
        default=None,
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        help="Path of the full dataset with all the patches",
        default=None,
    )
    parser.add_argument(
        "--weakly",
        type=int,
        help="Use weakly supervision. 1: YES; 0: NO ",
        default=0,
        choices=[0, 1],
    )
    parser.add_argument(
        "--train_split_ratio",
        type=float,
        help="Desired percentage for the training split.",
        default=0.8,
    )
    parser.add_argument(
        "--val_split_ratio",
        type=float,
        help="Desired percentage for the validation split.",
        default=0.1,
    )
    parser.add_argument(
        "--test_split_ratio",
        type=float,
        help="Desired percentage for the testing split.",
        default=0.1,
    )
    parser.add_argument(
        "--seed", type=int, help="Seed for random number generators.", default=42
    )
