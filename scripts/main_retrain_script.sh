...
models_list=("mod_unet" "mod_resunet" "mod_unet3+" "mod_attention_unet")
...
for band_list in "${band_combination_list[@]}"; do
	...
	for model in "${models_list[@]}"; do
		for n_filters in "${n_filters_list[@]}"; do
			if [ -z ${n_filters+x} ]; then n_filters_str="_$n_filters" ; else n_filters_str="_$n_filters" ; fi
